...
static const CGFloat kTopBackBottonLeft = 8.0;
static const CGFloat kTopBackButtonSize = 30.0;
 ...
 - (void)layoutSubviews
{
    ...
    if (_isFullScreen == YES) {
        ...
        [self.topBar.layer setMasksToBounds:NO];
        [self.topBar.layer setCornerRadius:0];
    }
    else {
        CGFloat TopBarBackX = kTopBackBottonLeft;
        CGFloat TopBarBackY = kTopBackBottonLeft;
        CGFloat TopBarBackSize = kTopBackButtonSize;
        self.topBar.frame = CGRectMake(TopBarBackX, TopBarBackY, TopBarBackSize, TopBarBackSize);
        [_topBar.layer setMasksToBounds:YES];
        [_topBar.layer setCornerRadius:self.topBar.frame.size.height / 2];
//        [_topBar.layer setBorderWidth:0.5];
        _topBar.contentView.backgroundColor = [UIColor colorWithRed:0.0795 green:0.1062 blue:0.1301 alpha:0.2];
    ...
    CGFloat backButtonX = 1.25;
    CGFloat backButtonY = 2.5;
    CGFloat backButtonSize = 0;

    if (_isFullScreen == YES) {
        self.backButton.hidden = YES;
    }else{
        backButtonImageName = @"bar-icon-back";
        backButtonSize = kBottomPlayButtonSize;
    }
    self.backButton.frame = CGRectMake(backButtonX, backButtonY, backButtonSize, backButtonSize);
    [self.backButton setImage:[UIImage imageNamed:backButtonImageName] forState:UIControlStateNormal];
 ...
 - (UIButton *)backButton
{
    if (!_backButton) {
        ...
        [_backButton setImage:[UIImage imageNamed:@"bar-icon-back"] forState:UIControlStateNormal];
        _backButton.bounds = CGRectMake(50, 50, kBottomPlayButtonSize_Little, kBottomPlayButtonSize_Little);
    }
    return _backButton;
}
